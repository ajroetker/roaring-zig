...
test "catch 'em all" {
    ...
    (try a._or(b)).free();
    a._orInPlace(b);
    var array = [_]*const Bitmap{ b, c, d };
    (try Bitmap._orMany(&array)).free();
    (try Bitmap._orManyHeap(&array)).free();
    _ = a._orCardinality(b);

    (try a._xor(b)).free();
    a._xorInPlace(b);
    _ = a._xorCardinality(b);
    (try Bitmap._xorMany(&array)).free();
...
fn iterate_sum(value: u32, data: ?*anyopaque) callconv(.C) bool {
    @as(*u32, @ptrCast(@alignCast(data))).* += value;
    return true;
}
